 ...
 */
package com.mydeliveries.toastit.imageview;
...
import com.mydeliveries.toastit.imageview.MultiTouchController.MultiTouchObjectCanvas;
import com.mydeliveries.toastit.imageview.MultiTouchController.PointInfo;
import com.mydeliveries.toastit.imageview.MultiTouchController.PositionAndScale;
...
public class PhotoSortrView extends View implements
	...
	MultiTouchEntity imgTemp;
	PointInfo touchPointTemp;
	...
	private ArrayList<Drawable> paths = new ArrayList<Drawable>();
	...
	public void addImages(Context context, Drawable resourceId) {
		Resources res = context.getResources();

		imageIDs.add(new ImageEntity(resourceId, res));
		paths.add(resourceId);
		float cx = getWidth() / 2;
		// SCREEN_MARGIN
	...
	public void DuplicateImage(Context context) {
		try {
			Resources res = context.getResources();
			//ImageEntity is = im.get;
			imageIDs.add(new ImageEntity(paths.get(paths.size() - 1), res));
			//imageIDs.add(is);
	...
	public void pushObjectUnder() {
		currTouchPoint.set(touchPointTemp);

		if (imgTemp != null) {

			// Move image to the top of the stack when selected
			drawPaint.setColor(Color.TRANSPARENT);
			imageIDs.remove(imgTemp);
			imageIDs.add(imgTemp);
